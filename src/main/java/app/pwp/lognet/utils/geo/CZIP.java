...
@Component
public class CZIP {
    private static Logger log = LoggerFactory.getLogger(CZIP.class);
     ...
     */
    private Map<String, CZIPLocation> ipCache;
    /**
     * 随机文件访问类
     */
     ...
     * @return
     */
    public synchronized CZIPLocation getIPLocation(final String ip) {
        CZIPLocation location = new CZIPLocation();
    ...
    public String getCountry(byte[] ip) {
        ...
        if (ipFile == null)
            return BAD_IP_FILE;
        // 保存ip，转换ip字节数组为字符串形式
        String ipStr = CZIPStringUtils.getIpStringFromBytes(ip);
        // 先检查cache中是否已经包含有这个ip的结果，没有再搜索文件
        if (ipCache.containsKey(ipStr)) {
            CZIPLocation ipLoc = ipCache.get(ipStr);
            return ipLoc.getCountry();
        } else {
            CZIPLocation ipLoc = getIPLocation(ip);
    ...
    public String getCountry(String ip) {
        return getCountry(CZIPStringUtils.getIpByteArrayFromString(ip));
    }
    ...
    public String getArea(final String ip) {
        return getArea(CZIPStringUtils.getIpByteArrayFromString(ip));
    }
     ...
     * @return IPLocation结构
     */
    private CZIPLocation getIPLocation(final byte[] ip) {
        CZIPLocation info = null;
        long offset = locateIP(ip);
        if (offset != -1)
            info = getIPLocation(offset);
        if (info == null) {
            info = new CZIPLocation();
     ...
     * @return IPLocation对象
     */
    private CZIPLocation getIPLocation(long offset) {
        try {
            // 跳过4字节ip
     ...
     * @return IPLocation对象
     */
    private CZIPLocation getIPLocation(int offset) {
        // 跳过4字节ip
    ...
    private String readString(long offset) {
        try {
            ...
            if (i != 0)
                return CZIPStringUtils.getString(buf, 0, i, "GBK");
        } catch (IOException e) {
            log.error(e.getMessage(), e);
        }
        return "";
